...
class AirVisualCard extends HTMLElement {
  ...
  set hass(hass) {
    ...
    const mainPollutantValueSensor = { name: 'mainPollutantValueSensor', config: config.main_pollutant_value || null, value: '' }
    const sensorList = [aqiSensor, aplSensor, mainPollutantSensor, mainPollutantValueSensor];
    ...
    for (i = 0; i < sensorList.length; i++) {
      ...
      if (typeof hass.states[mainPollutantValueSensor.config] != "undefined") {
        mainPollutantValueSensor.value = hass.states[aqiSensor.config].state;
      }
    };
    ...
    if (!hideAPL) {
      card_content += `
          ...
          <div class="mainPollutantSensor" id="mainPollutantSensor" style="background-color: ${AQIbgColor[getAQI()]}; color: ${AQIfontColor[getAQI()]}">
                <div class="mainPollutantSensorText">${mainPollutant} | ${mainPollutantValueSensor.value}${pollutantUnit}</div>
          </div>
        `;
    }
    card_content += `
